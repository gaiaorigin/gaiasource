...
class source(provider.ProviderBase):
	...
	def __init__(self):
		...
		self.priority = 0
		self.language = ['un']
		self.domains = ['zooqle.me', 'zooqle.com']
		self.base_link = 'https://zooqle.me'
		self.search_link = '/search?pg=%d&q=%s%%20category%%3A%s&s=ns&v=t&sd=d'
		self.category_movies = 'Movies%2CAnime'
		self.category_shows = 'TV%2CAnime'
		...
		try:
			...
			while True:
				...
				for i in range(1, len(htmlRows)): # First row is header.
					...
					# Name
					htmlName = htmlInfo.find_all('a', recursive = False)[0].getText().strip()
					...
					# Seeds
					htmlSeeds = htmlColumns[5].find_all('div', recursive = False)[0]['title']
					indexStart = htmlSeeds.find(':')
					if indexStart > 0:
						indexStart += 1
						indexEnd = htmlSeeds.find('|', indexStart)
						if indexEnd > 0: htmlSeeds = htmlSeeds[indexStart:indexEnd]
						else: htmlSeeds = htmlSeeds[indexStart:]
						htmlSeeds = int(htmlSeeds.replace(',', '').replace('.', '').strip())
					else:
						htmlSeeds = None

					# Quality & 3D
					try:
						htmlQuality = htmlMeta.find_all('span', class_ = 'hidden-xs')[0].getText().lower().strip()
						if 'ultra' in htmlQuality: htmlQuality = '4K'
						elif 'std' in htmlQuality: htmlQuality = 'SD'
						elif 'med' in htmlQuality or 'low' in htmlQuality: htmlQuality = 'CAM'
						htmlName += ' ' + htmlQuality
					except: pass

					# Audio
					try: htmlName += ' ' + htmlMeta.find_all('span', {'title' : 'Audio format'})[0].getText()
					except: pass

					# Languages
					try:
						htmlLanguages = htmlMeta.find_all('span', {'title' : 'Detected languages'})[0].getText().split(',')
					except:
						htmlLanguages = None

					# Metadata
					meta = metadata.Metadata(name = htmlName, title = title, titles = titles, year = year, season = season, episode = episode, pack = pack, packCount = packCount, link = htmlLink, size = htmlSize, seeds = htmlSeeds, languageAudio = htmlLanguages)
