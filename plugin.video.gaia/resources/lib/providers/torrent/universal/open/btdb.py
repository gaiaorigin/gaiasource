...
class source(provider.ProviderBase):
	...
	def __init__(self):
		...
		self.base_link = 'https://btdb.to'
		self.search_link = '/q/%s/%i?sort=popular'
	...
	def sources(self, url, hostDict, hostprDict):
		...
		try:
			...
			while True:
				...
				htmlRows = html.find_all('li', class_ = 'search-ret-item')
				for i in range(0, len(htmlRows)):
					try:
						htmlRow = htmlRows[i]
						htmlMeta = htmlRow.find_all('div', class_ = 'item-meta-info')[0]

						# Name
						htmlName = htmlRow.find_all('h2', class_ = 'item-title')[0].getText().strip()

						# Size
						htmlSize = htmlMeta.find_all('span', class_ = 'item-meta-info-value')[0].getText().strip()

						# Link
						htmlLink = htmlMeta.find_all('a', class_ = 'magnet')[0]['href'].strip()

						# Seeds
						# No seeds shown.
						# Use the popularity instead.
						# Devide by 400 seems to be close to the actual seeds.
						htmlSeeds = int(float(htmlMeta.find_all('span', class_ = 'item-meta-info-value')[3].getText().strip()) / 400.0)
