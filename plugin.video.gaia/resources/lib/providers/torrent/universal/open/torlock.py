...
class source(provider.ProviderBase):
	...
	def __init__(self):
		...
		self.search_link = '/%s/torrents/%s.html?sort=seeds&page=%i'
	...
	def sources(self, url, hostDict, hostprDict):
		...
		try:
			...
			while True:
				...
				htmlTable = html.find_all('div', class_ = 'panel')[0].find_all('table', recursive = False)[0]
				...
				for i in range(len(htmlRows)):
					...
					# Size
					htmlSize = htmlRow.find_all('td', class_ = 'ts', recursive = False)[0].getText().strip()
					...
					# Seeds
					htmlSeeds = int(htmlRow.find_all('td', class_ = 'tul', recursive = False)[0].getText().strip())
