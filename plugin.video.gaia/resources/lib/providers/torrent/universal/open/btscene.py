...
class source(provider.ProviderBase):
	...
	def __init__(self):
		...
		self.priority = 0
		self.language = ['un']
		self.domains = ['btstors.com', 'bt-scene.cc']
		self.base_link = 'http://btstors.com'
		self.search_link = '/search?q=%s&p=%d'
	...
	def sources(self, url, hostDict, hostprDict):
		...
		try:
			...
			while True:
				...
				htmlTable = html.find_all('table', class_ = 'rtable')[0].find_all('tbody', recursive = False)[0]
				htmlRows = htmlTable.find_all('tr', class_ = 'row')
				...
				for i in range(1, len(htmlRows)):
					...
					# Link
					htmlLink = htmlColumns[5].find_all('a')[0]['href'].strip()
