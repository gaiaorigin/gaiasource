...
class source(provider.ProviderBase):
	...
	def __init__(self):
		...
		self.search_link = '/search_results.php?incldead=0&inclexternal=0&lang=0&sort=seeders&order=desc&cat=%d&search=%s&page=%d'
		self.category_movies = 1
		self.category_shows = 41
	...
	def sources(self, url, hostDict, hostprDict):
		...
		try:
			...
			while True:
				...
				htmlRows = htmlTable.find_all('tr', class_ = 't-row', recursive = False)
				for i in range(len(htmlRows)):
					try:
						...
						# Name
						htmlName = htmlColumns[1].getText().strip()

						# Size
						htmlSize = htmlColumns[4].getText().strip()

						# Link
						htmlLink = htmlColumns[3].find_all('a')[0]['href']

						# Seeds
						htmlSeeds = int(re.sub('[^0-9]', '', htmlColumns[5].getText().strip()))
