...
class source(provider.ProviderBase):
	...
	def __init__(self):
		...
		self.priority = 0
		self.language = ['un']
		self.domains = ['torrentdownload.ch']
		self.base_link = 'http://www.torrentdownload.ch'
	...
	def sources(self, url, hostDict, hostprDict):
		...
		try:
			...
			while True:
				...
				htmlTable = html.find_all('table', class_ = 'table2')[1] # First table is the direct-link table.
				htmlRows = htmlTable.find_all('tr', recursive = False)
				for i in range(1, len(htmlRows)):
					htmlRow = htmlRows[i]

					# Name
					htmlName = htmlRow.find_all('td', class_ = 'tdleft', recursive = False)[0].find_all('div', class_ = 'tt-name', recursive = False)[0].getText().strip()

					# Size
					htmlSize = htmlRow.find_all('td', class_ = 'tdnormal', recursive = False)[1].getText().strip()

					# Link
					htmlLink = htmlRow.find_all('td', class_ = 'tdleft', recursive = False)[0].find_all('div', class_ = 'tt-name', recursive = False)[0].find_all('a', recursive = False)[0]['href']
					htmlLink = network.Container(htmlLink.split('/')[1]).torrentMagnet(title = title)

					# Seeds
					htmlSeeds = int(htmlRow.find_all('td', class_ = 'tdseed', recursive = False)[0].getText().replace(',', '').strip())
