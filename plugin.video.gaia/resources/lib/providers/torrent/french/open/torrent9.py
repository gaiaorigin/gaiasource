...
class source(provider.ProviderBase):
	...
	def __init__(self):
		...
		self.priority = 0
		self.language = ['fr']
		self.domains = ['torrent9.cz', 'torrent9.uno']
		self.base_link = 'https://www.torrent9.cz'
		self.search_link = '/recherche/%s/%d'
		self.type_movies = 'films'
		self.type_shows = 'séries'
	...
	def _link(self, url, index):
		try:
			...
			links = []
			for i in buttons:
				try: links.append(i.find_all('a')[0].get('href'))
				except: pass
			for i in links:
	...
	def _search(self, url, query, show, type, title, titles, year, season, episode, pack, packCount, packException, ignoreContains):
		...
		try:
			while True:
				...
				html = BeautifulSoup(client.request(url % (query, page)))

				page += 50
				added = False

				htmlTable = html.find_all('table', class_ = 'cust-table')
				if len(htmlTable) > 0:
					...
					for i in range(len(htmlRows)):
						...
						# Category
						htmlCategory = htmlColumns[0].find_all('i')[0].get('class')
						htmlCategory = [category.encode('utf-8').lower() for category in htmlCategory]
						if not type in htmlCategory: continue
						...
						# Size
						htmlSize = re.sub('([mMkKgGtT]?)[oO]', '\\1b', htmlColumns[1].getText())
