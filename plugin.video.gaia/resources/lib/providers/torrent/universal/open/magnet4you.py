...
class source(provider.ProviderBase):
	...
	def __init__(self):
		...
		self.search_link = '/search.php?s=%s&sort=seed&start=%i'
	...
	def sources(self, url, hostDict, hostprDict):
		...
		try:
			...
			while True:
				...
				data = data.replace('</a></form></td>', '</a></td>') # There are a stand-alone closing form tags in the table.
				...
				htmlRows = html.find_all('table', class_ = 'tb4')
				for i in range(0, len(htmlRows)):
					try:
						...
						# Name
						htmlName = htmlColumns[0].find_all('a', recursive = False)[1].getText().strip()

						# Size
						htmlSize = htmlColumns[2].getText().strip()

						# Link
						htmlLink = htmlColumns[0].find_all('a', recursive = False)[0]['href'].strip()
