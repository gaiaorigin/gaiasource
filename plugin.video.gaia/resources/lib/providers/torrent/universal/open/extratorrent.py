...
class source(provider.ProviderBase):
	...
	def __init__(self):
		...
		self.priority = 0
		self.language = ['un']
		self.domains = ['extratorrent.cd']
		self.base_link = 'https://extratorrent.cd'
		self.search_link = '/search/?search=%s&s_cat=&pp=&srt=seeds&order=desc'
	...
	def sources(self, url, hostDict, hostprDict):
		...
		try:
			...
			htmlTable = html.find_all('table', class_ = 'tl')[0]
			...
			for i in range(1, len(htmlRows)): # First row is header.
				...
				# Name
				htmlName = htmlColumns[2].find_all('a', recursive = False)[0].getText().strip()

				# Size
				htmlSize = htmlColumns[4].getText().strip()

				# Link
				htmlLink = htmlColumns[0].find_all('a', recursive = False)[1]['href']
